...
export default function CCOEmployeesPage() {
    ...
    useEffect(() => {
        // CCO needs to see ALL users, so we pass undefined for department filter
            ...
            <main className="p-4 md:ml-64 md:p-8 space-y-12 animate-in fade-in duration-500 pb-20">
                ...
                {departments.map(dept => {
                    const deptUsers = users.filter(u =>
                        dept === "Other"
                            ? (!u.department || !["Development", "UX", "Social Media"].includes(u.department as string))
                            : u.department === dept
                    );
