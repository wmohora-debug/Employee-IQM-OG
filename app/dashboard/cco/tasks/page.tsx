...
export default function CCOTasksPage() {
    ...
    // Data Subscription
    useEffect(() => {
        if (loading || user?.role !== 'cco') return;
    ...
    const deptStats = ["Development", "UX", "Social Media"].map(dept => {
        // Exclude CEO/CCO/COO tasks from workload stats? "Operational Tasks" usually means excluding Board directives.
        // Let's exclude tasks created by 'cco' for CCO dashboard?
        // Or exclude all executive tasks?
        // Let's follow CEO pattern: exclude tasks created by ANY EXECUTIVE (to separate strategic from operational).
    ...
    const groupedTasks = {
        ...
        'Social Media': tasks.filter(t => t.department === 'Social Media' && !['ceo', 'cco', 'coo'].includes(t.createdByRole || ''))
    };
    ...
    if (user?.role !== 'cco') return null;

    return (
        <>
            <Header title="Tasks Overview (CCO)" />
